...
public class SelectionManager : MonoBehaviour
{
    ...
    private void SelectUnitsInDrag()
    {
        ...
        {
            if(UnitManager.instance.units[i].GetComponent<UnitTasks>().renderer.isVisible)
            {
    ...
    public void MakeAllJobBoolsFalse()
    {
        for(int i = 0; i < currentlySelectedUnits.Count; i++)
        {
            currentlySelectedUnits[i].GetComponent<UnitTasks>().isIdle = false;
            currentlySelectedUnits[i].GetComponent<UnitTasks>().isWoodcutter = false;
            currentlySelectedUnits[i].GetComponent<UnitTasks>().isMiner = false;
        }
    }
